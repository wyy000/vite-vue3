...
table(:class="['w-full table-auto', hoverType ? 'hover-' + hoverType : '']")
  ...
  thead
    tr
      td(v-for="(col, idx) of meta.columns" :key="idx" :col="col" :class="[{'middle_child': idx !== 0 && idx !== meta.columns.length - 1}]")
        div(class="z-0")
  ...
  tbody(v-for="(sections, idx) in tbodiesData")
    ...
    tr(v-for="(row, rIdx) of sections.data" :key="rIdx")
      td(v-for="(col, cIdx) of meta.columns" :key="cIdx" class="h-full")
...
import ATablePaging from './a-table-paging.vue'
...
// TODO 左右col固定,指定首列序列号,添加demo操作,z-index值覆盖问题,外层边框1px问题...背景色问题
</script>
